...
// Componente LoadingButton (agregar al inicio del archivo)
...
// Componente ErrorMessage (agregar después de LoadingButton)
...
export default function PaymentPlans() {
  const { user } = useUser();
  const [loading, setLoading] = useState(null); // Cambiamos a null para trackear qué botón está loading
  ...
  const handleUpgrade = async (planId) => {
    ...
    try {
      setLoading(planId); // Setear qué plan está cargando
      ...
      setError(error.message); // Setear el error para mostrar el componente
    } finally {
      setLoading(null); // Limpiar loading state
    }
  };
  ...
  return (
    <div className='max-w-6xl mx-auto px-4 py-12'>
      ...
      {/* AGREGAR: Mostrar error si existe */}
